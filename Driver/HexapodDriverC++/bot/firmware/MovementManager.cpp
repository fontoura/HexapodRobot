...
#include "./MagnetometerAdjuster.h"
#include "./movement/movements.h"
#include "./movement/HaltOperation.h"
#include "./movement/AdjustOperation.h"
...
namespace bot
{
	namespace firmware
	{
		...
		void MovementManager::initialize(_strong(RobotManager)& robot)
		{
			...
			m_driver = InterpolationDriver::create();
			initPosition(m_driver.get());
			m_currentId = 0;
			m_robot = robot;
		...
		MovementManager* MovementManager::create(_strong(RobotManager)& robot)
		{
			...
			_new_inst(MovementManager, manager);
		...
		void MovementManager::run()
		{
			...
			while (true)
			{
				if (m_currentOperation != NULL)
				{
					...
					m_currentOperation->run(m_currentId, _this);
					if (m_isAdjusting)
					{
		...
		_strong(InterpolationDriver)& MovementManager::getDriver()
		{
			return m_driver;
		}
	}
