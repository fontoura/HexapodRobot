...
namespace bot
{
	...
	{
		/* pool de objetos. */
		ObjectPool<UartManager, 1> UartManager::m_pool;
		...
		void UartManager::finalize()
		{
			...
			UartManager::m_pool.recycle(this);
		}
		...
		UartManager* UartManager::create(_strong(RobotManager)& robot, _strong(Flow)& flow)
		{
			...
			UartManager* uartManager = UartManager::m_pool.obtain();
			if (uartManager != NULL)
			{
			...
			if (request->getLength() >= 0x6)
			{
				...
				switch (readLittleEndian16(request->getBuffer(), 0))
				{
					...
					case MOVEMENT_LOOKTO:
						if (request->getLength() >= 0xa)
						{
							DEBUG3("UartManager recebeu request de definir movimento para olha para direcao com ID ", label, ".");
							int angle = readLittleEndian32(request->getBuffer(), 0x6) / 1024;
							if (angle < 0) {
								angle = 360 + angle;
							}
							MagnetometerData data;
		...
		void UartManager::sendNotification(_strong(Message)& notification)
		{
			...
			_strong(NotificationSender) sender = NotificationSender::create(8, m_flow, _this, 1000);
